...
def sales_output(
    ...
    result = PolarsFilterProcessor("sale_price_usd", MIN_SALE_VALUE_USD, ">=").process(
        sales_transform
    )
...
def artworks_transform(context: dg.AssetExecutionContext, paths: PathsResource) -> pl.DataFrame:
    ...
    sales_per_artwork = DuckDBQueryProcessor(
        sql=f"""
        SELECT
            s.artwork_id,
            COUNT(*) AS sale_count,
...
def artworks_output(
    ...
    context.log.info(
        f"Output {len(artworks_transform)} artworks to {artworks_path} in {elapsed_ms:.1f}ms"
    )
